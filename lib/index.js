...
internals.getNextFileSequence = function (directory, baseFileName, callback) {
...
module.exports = internals.GoodFile = function (path, events, options) {
    ...
    if (settings.rotationTime) {
        ...
        settings.maxLogSize = Infinity;
        delete settings.rotationTime;

        var filePath = Path.join(path, settings.fileName);
        ...
        this._baseName = Path.basename(path);
        this._getFileName = internals.namedFile.bind(this, this._directory, this._baseName);
    }
};
...
internals.GoodFile.prototype.start = function (emitter, callback) {
    ...
    var buildWriteStream = internals.buildWriteStream.bind(null, emitter, this._state.eventHandler);

    this._queue = Async.queue(function (data, next) {
    ...
    if (this._settings.fileName) {
        ...
        this._currentStream = buildWriteStream(fileName, this._queue);
        return process.nextTick(callback);
    }
    else if (!this._baseName) {
        var fileName = this._getFileName();
        this._currentStream = buildWriteStream(fileName, this._queue);

        if (this._settings.rotationTime) {
            this._state.interval = setInterval(function () {
        ...
        internals.getNextFileSequence(this._directory, this._baseName, function (err, num) {

            if (err) {
                return callback(err);
            }

            self._nextNumber = internals.nextFileNumber(num);
